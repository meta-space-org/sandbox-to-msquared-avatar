...
import { normals, prune, weld, unpartition, transformMesh, mergeDocuments, joinPrimitives } from '@gltf-transform/functions';

import { Packer, iterateTextures } from './includes/packer.js';
import { Atlas, remapUvsToAtlas } from './includes/atlas.js';
import { inspect, humanFileSize } from './includes/utils.js';
...
/**
 * Convert input sandbox GLTF file into MSquared GLB file
 * 
 * @param {Object} options 
 * @param {String} options.file Full path to file for processing
 * @param {String} options.output Full path to output LGB file
 * @param {Boolean} [options.merge] If true, all meshes will be merged into a single model
 * @param {Boolean} [options.silent] If true, the process will not console log info
 * @param {Boolean} [options.inspect] If true, the detailed GLB info will be logged
 * @returns {Boolean}
 */
const convert = async function(options) {
    ...
    for(let i = 0; i < nodes.length; i++) {
        ...
        if (!mesh && node.getName().endsWith('-Local')) {
    ...
    if (options.merge) {
        ...
        // const emissiveStrength = emissiveStrengthExtension.createEmissiveStrength().setEmissiveStrength(2.0);
    ...
    await main.transform(normals({ overwrite: true }));
